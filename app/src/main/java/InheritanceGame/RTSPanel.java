...
public class RTSPanel extends JPanel {
	...
	private class MoveListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			// alfred.setVelocity(10);
			// alfred.setDirectionH(0);
			// alfred.setDirectionV(1);

			// for (UnitAbstract ua : units) {
			// if (ua.getxLoc() < MIN_HORIZONTAL) {
			// ua.setDirectionH(1);
			// }
			//
			// if (ua.getxLoc() > MAX_HORIZONTAL - ua.getScaleX()) {
			// ua.setDirectionH(0);
			// }
			//
			// if (ua.getyLoc() < MIN_VERTICAL) {
			// ua.setDirectionV(1);
			// }
			//
			// if (ua.getyLoc() > MAX_VERTICAL - ua.getScaleY()) {
			// ua.setDirectionV(0);
			// }
			// ua.move();
			for (UnitAbstract ua : units) {
